...
function Client() {
	...
	// public methods
	self.send = function(type, param, to) {
		if (!to) to = null; // null means "broadcast"
		self.emit("send", {
			type: type,
			body: param,
			dest: {
...
function CometClient() {
		...
		get: (function() {
			var option = {
				...
				success: function(packet) {
					if (!packet || !packet.queue || !$.isArray(packet.queue))
						return setTimeout(this._processedCallback, 0);;
					
					var list = packet.queue;
					for (var n in list) {
						self.emit("receive", list[n]);
						queue.push(list[n]);
					}
	...
	self.on("transaction", function(packet) {
		comet.post(path, {
			mode: "transaction",
			data: packet
		});
...
window[namespaces] = {
	...
	require: function(path) {
		...
		if (req.status != 200) {
			req.abort();
			throw new Error("failed to load remote script file. ["+fileName+"]");
		}
